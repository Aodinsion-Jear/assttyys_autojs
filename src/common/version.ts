...
修复: 401对弈竞猜庭院右侧活动不翻页问题，误识别体服问题`
	...
	version: '20260807_tyfs_01',
	desc: '同步原作者更新至350da25d：修复016地鬼第三个未开启时卡住/打完一个不返回、508逢魔首领界面不点击挑战、998寄养己方结界优化取点，新增定时任务推送配置与运行时间统计；008寮突破改用原作者取色(若体服失效可回滚至20260721_tyfs_02取色：比色[left,27,518,0xb9b9b9]等6点、点击区[right,1206,383,1251,475])；保留029突破界面退出自定义与本地前端依赖'
}, {
	version: '20260807_tyfs_02',
	desc: '新增703武道大会-探索：已探索出怪物则点击左侧怪物图标进入挑战界面，未探索则点击右侧搜寻按钮(探索后自动进入挑战界面)，已在挑战界面时不重复操作。需自行新建方案勾选该功能'
}, {
	...
	desc: '同步原作者前端更新至9e96563(前端1.0.11->1.0.14)：定时任务新增推送配置(JobOptions.pushOption)、修复导入方案缺commonConfig无法修改、修复mock模式定时任务无初始数据与cron模式下次执行时间不生效；零冲突，已保留移除OAuth验证页与本地前端依赖。703武道大会-探索术语统一为御灵并补充需配合通用活动使用的说明，仅文字无逻辑改动'
}, {
	version: '20260813_01',
	...
	version: '20260826_tyfs_01',
	desc: '同步原作者更新至2d04d9aa：修复契灵不识别界面、优化悬赏直接结束，703武道大会修复搜寻时点击切换门票/部分设备不识别场景并增加连选5次自动停止；703武道大会整体采用上游版本(放弃体服取色适配)，锁文件采用上游版本；保留移除OAuth验证与本地前端依赖'
}, {
	version: '20260827_01',
	desc: '新增：庭院皮肤"缘结之庭"兼容（庭院菜单展开/未展开判定、探索灯笼、町中竖牌、悬赏图标，流畅画质取色）。\n重构：庭院皮肤兼容改为按皮肤分文件聚合结构（src/common/courtSkins/），找色desc支持变体元数据与点击偏移，后续新增皮肤只需添加数据文件。'
}, {
	version: '20260901_01',
	desc: '新增：519寮管理攻打道馆支持按指定道馆名称攻打（配置道馆名称后OCR匹配列表并选中，可填部分名字，找不到时刷新3次后停止）。\n优化：6120庭院进入狩猎战改为庭院进入寮神社，新增配置可选进入麒麟或道馆（默认麒麟，行为不变）。\n优化：018每日悬赏停止时推送与toast附带停止原因。'
}, {
	version: '20260902_01',
	...
	version: '20260912_tyfs_06',
	desc: '新增：6120庭院进入寮神社新增阴界之门选项；新增6121阴门挑战（地图点阴门图标→挑战界面点挑战→弹窗点确认→进组队界面移交6231，识别到已挑战或30s无识别则停止脚本）；新增6231阴门组队点击挑战。'
}, {
	version: '20260914_tyfs_01',
	desc: '优化：6130挑战麒麟重构，更新星级选择页、挑战页与协战提示取色；选择六星后通过OCR确认完整“当前选择”再开启挑战（需启用OCR扩展），识别最多尝试3次，失败提示并停止。\n修复：未借用协战式神提示改为点击确定继续挑战；保留麒麟已击杀后返回并停止逻辑。适用于“庭院进入狩猎战”方案。'
}, {
	version: '20260914_tyfs_02',
	desc: '优化：麒麟收尾流程改造——6130识别到麒麟已击杀/已挑战不再直接停止脚本，改为打标记并移交503返回庭院（503新增“麒麟已挑战界面”返回分支）；修复返回途中6120抢先导航进寮、503来不及确认庭院的竞态（6120见到结束标记即停手，标记按方案名隔离，不影响其他方案）；“庭院进入狩猎战”方案功能列表加入503（老方案需在方案管理中手动添加503）。'
}];
